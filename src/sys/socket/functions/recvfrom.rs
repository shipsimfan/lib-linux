...
use crate::{
    errno::{
        EAGAIN, EBADF, ECONNREFUSED, EFAULT, EINTR, EINVAL, ENOTCONN, ENOTSOCK, EWOULDBLOCK, errno,
    },
    sys::socket::{
        AF_PACKET, AF_UNSPEC, MSG_DONTWAIT, MSG_ERRQUEUE, MSG_OOB, MSG_PEEK, MSG_TRUNC,
        MSG_WAITALL, accept, connect,
    },
};
#[allow(unused_imports)]
use std::ptr::null;
...
unsafe extern "C" {
    ...
    ///
    /// [`recvfrom`] returns the length of the message on successful completion. If a message is
    ...
    ///
    /// The flags argument to a [`recvfrom`] call is formed by ORing one or more of the following
    /// values:
