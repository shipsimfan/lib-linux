...
extern "C" {
    ...
    /// the arguments `dest_addr` and `addrlen` are ignored (and the error EISCONN may be returned
    /// when they are not [`null`] and 0), and the error [`ENOTCONN`] is returned when the socket
    /// was not actually connected. Otherwise, the address of the target is given by `dest_addr`
